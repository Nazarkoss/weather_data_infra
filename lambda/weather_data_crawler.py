    ...
    if event:
        ...
        for message in event["Records"]:
            try:
                ...
                hourly_dataframe['date'] = hourly_dataframe['date'] = hourly_dataframe['date'].dt.tz_localize(None).astype('datetime64[ms]')
                ...
                buffer = BytesIO()
                hourly_dataframe.to_csv(buffer)

                # Upload to S3
                s3.put_object(Bucket=bucket_name, Key=f"raw/data-around-france/weather_lat_{latitude}_lon_{longitude}_{start_date}-{end_date}.csv", Body=buffer.getvalue())
        ...
        if batch_item_failures:
            print(f"FAILURES: {sqs_batch_response}")
